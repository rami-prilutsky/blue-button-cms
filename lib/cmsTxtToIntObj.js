...
function separateSections(data) {
  ...
  ignore capitalization */
    ...
    // some texts don't send with hyphens
    //data = data.trim().endsWith('---') ? data : data + '-'.repeat(33);
    //data = data.trim().indexOf('---', data.length - 3) !== -1 ? data : data + '-'.repeat(33);
    data = data.trim().indexOf('---', data.length - 3) !== -1 ? data : data + (new Array(33).join("-"));
